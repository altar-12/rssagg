...
const fetchUserFeeds = `-- name: FetchUserFeeds :many
SELECT id, created_at, updated_at, name, url, user_id, last_fetched_at FROM feeds WHERE user_id = $1
`

func (q *Queries) FetchUserFeeds(ctx context.Context, userID uuid.UUID) ([]Feed, error) {
	rows, err := q.db.QueryContext(ctx, fetchUserFeeds, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
