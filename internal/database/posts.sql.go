...
FROM
(SELECT id, created_at, updated_at, user_id, feed_id FROM feed_follows WHERE user_id = $1) AS feed_follows JOIN posts
ON feed_follows.feed_id = posts.feed_id
ORDER BY posts.created_at DESC
OFFSET $2 LIMIT $3
`

type FetchUserPostsParams struct {
	UserID uuid.UUID
	Offset int32
	Limit  int32
}

func (q *Queries) FetchUserPosts(ctx context.Context, arg FetchUserPostsParams) ([]Post, error) {
	rows, err := q.db.QueryContext(ctx, fetchUserPosts, arg.UserID, arg.Offset, arg.Limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
